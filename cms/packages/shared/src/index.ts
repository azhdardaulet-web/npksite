...
// ─── ИИН (Казахстан: 12 цифр, ГГММДД + век/пол + порядковый номер + контрольная цифра) ─
// Используется на форме вступления в партию (/vstupit, шаг «Данные»).
...
export function isValidIin(iin: string): boolean {
  ...
  // Контрольная сумма: первый проход — веса 1..11, если остаток 10 — второй проход со сдвинутыми весами.
...
export const iinSchema = z
  ...
  .refine(isValidIin, 'Неверный ИИН (ошибка в контрольной сумме или дате рождения)');

// ─── Номер удостоверения личности РК (9 цифр) ──────────────────────────────────

export const idDocNumberSchema = z
  .string()
  .trim()
  .regex(/^\d{9}$/, 'Номер удостоверения — 9 цифр');
