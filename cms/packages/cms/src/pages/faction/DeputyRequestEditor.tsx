...
export default function DeputyRequestEditor() {
  ...
  const save = async () => {
    if (!form.title.trim() || !form.fileUrl.trim()) {
      setError('Заполните заголовок и ссылку на полный текст');
      return;
    }
    setSaving(true);
    setError('');
        ...
        <div className="space-y-4">
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <div className="flex items-center border-b border-gray-200">
              <button type="button" className="flex items-center gap-2 px-4 py-3 text-sm font-medium border-b-2 border-red-600 text-red-600 bg-red-50">
                РУ
                {form.title.trim() && <span className="w-1.5 h-1.5 rounded-full bg-green-500" />}
              </button>
            </div>
            <div className="p-5 space-y-4">
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">Заголовок (обязательно)</label>
                <input
                  value={form.title}
                  onChange={(event) => setForm((current) => ({ ...current, title: event.target.value }))}
                  placeholder="Заголовок депутатского запроса..."
              ...
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">Текст и краткое описание запроса</label>
                <RichTextEditor
                  value={form.description}
                  onChange={(description) => setForm((current) => ({ ...current, description }))}
                  placeholder="Введите содержание депутатского запроса..."
                  minHeight={320}
                />
              </div>
            </div>
