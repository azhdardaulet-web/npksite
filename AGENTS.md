# AGENTS.md — НПК Сайт (npksite)

Контекст для Codex при работе с этим репозиторием. Основной план работ ведётся в [docs/PLAN.md](docs/PLAN.md) — начинай с него, если он существует, и обновляй по мере продвижения по задачам.
...
| Папка | Что это | Стек |
|---|---|---|
| `app/` | Фронтенд сайта партии (SPA) — основная часть, с которой обычно ведётся работа | React 19 + TypeScript + Vite |
| `cms/` | **Активно дорабатываемый backend + админка CMS/CRM** для НПК. Изначально код перенесён из другого проекта автора (DAR Rail CMS) — сейчас адаптируется под НПК: Prisma-схема, роли, роуты постепенно заменяются на модель НПК (см. [docs/PLAN.md](docs/PLAN.md), Часть 1–2). Часть модулей/моделей может ещё носить старые названия DAR Rail до соответствующего этапа плана | Node.js 20 + Express 5 + Prisma + PostgreSQL 16 (`packages/api`), React 19 + Vite + Tailwind (`packages/cms`), общие Zod-схемы (`packages/shared`), MinIO (файлы), Docker + nginx |
| `content/` | Исходные медиафайлы (шрифты, логотипы, фото, видео) для загрузки в CMS/сайт | статика |

Актуальная CMS/CRM-архитектура (адаптация `cms/`, роли, схема БД, интеграции SMS/Telegram/Codex API/Daily.co) описана в [docs/PLAN.md](docs/PLAN.md) — это основной план работ по Блоку 2 ТЗ, следуй ему пошагово (Этапы 0–9). Секреты `cms/` — только в `cms/.env` (не коммитить, уже в `.gitignore`).
