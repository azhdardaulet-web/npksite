    ...
    <section className="bg-coal py-20 md:py-28 overflow-hidden">
      <div className="max-w-[1280px] mx-auto px-4 md:px-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-start">
          {/* Left: text + contacts */}
          <div>
            <p className="text-label font-light text-fog mb-3">Контакты</p>
            <TextReveal
              tag="h2"
                ...
                return (
                  <ScrollReveal key={contact.label} delay={i * 0.1}>
                    <div className="flex items-start gap-5">
                      <div className="w-12 h-12 rounded-full bg-ash border border-white/[0.08] flex items-center justify-center shrink-0">
                        <Icon size={20} strokeWidth={1.5} className="text-white" />
                      </div>
                      <div>
                        <p className="text-[20px] font-bold text-white mb-1">{contact.label}</p>
                        <a
                          href={contact.href}
                          target={contact.href.startsWith('http') ? '_blank' : undefined}
                          rel={contact.href.startsWith('http') ? 'noopener noreferrer' : undefined}
                          className="text-body font-light text-fog hover:text-white transition-colors"
                        >
                          {contact.value}
                        </a>
                      </div>
          ...
          {/* Right: map */}
          <div className="relative aspect-[4/3] lg:aspect-auto lg:h-[480px] rounded-card overflow-hidden bg-ash">
            <iframe
